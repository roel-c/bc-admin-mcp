...
| `b2b/invoices/*`, `b2b/receipts/*` | R0 reads; **R2** create/create_from_order/update; **R3** delete (invoices), **R3**/**R2** delete (receipts/lines); served from a distinct `/ip` base URL, not the standard B2B base — see `b2bBaseURL` usage in `internal/bigcommerce/b2b_invoices.go`. `create_from_order` takes the BC order ID but must call the API with B2B Edition's internal order ID — resolved automatically via `GetB2BOrder` | `internal/tools/b2b/invoice_tools.go` |
| `b2b/payment_records/*` | R0 reads; **R2** create_offline/update_offline/perform_operation/update_processing_status; **R3** delete; also served from the `/ip` base | `internal/tools/b2b/payment_record_tools.go` |
| `b2b/payments/*`, `b2b/companies/payments\|credit\|payment_terms/*` | R0 reads; **R2** company payments/credit/payment_terms updates; global `/payments` and per-company `/companies/{id}/payments` use different field names for the same data | `internal/tools/b2b/payment_tools.go` |
